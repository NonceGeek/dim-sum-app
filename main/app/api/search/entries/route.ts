...
export async function GET(req: NextRequest) {
  return publicApi(req, async () => {
    ...
    const rows = await fetchAggregatedSearchRows({
      query,
      similarOffset,
      recommendedOffset,
    });

    const primaryRow = rows.find((row) => row.section === "primary");
    ...
    const similarRows = rows.filter((row) => row.section === "similar");
    const recommendedRows = rows.filter((row) => row.section === "recommended");
