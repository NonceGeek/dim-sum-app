drop function if exists public.get_entry_identities(uuid[]);

create function public.get_entry_identities(p_unique_ids uuid[])
returns table (
  id bigint,
  unique_id uuid,
  data text,
...
as $$
  with selected_rows as (
    select
      ...
      cc.editable_level::bigint as editable_level,
      c.lifecycle_stage,
      c.liked_num,
      c.bookmark_num,
      c.view_num,
...
comment on function public.get_entry_identities(uuid[]) is
  'Batch entryIdentity aggregation for search, SEO, sharing, editing permission, and future Supabase RPC clients.';
