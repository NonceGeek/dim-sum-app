...
封面图生成是给一个提示词（可以是活动的主题之类的），我这里扩写，然后返回4 张候选，是临时的 URL，需要在选中作为封面时上传到项目自己的 OSS 里。

投稿提交时的内容安全检查，为了快一点返回，就只检查文本和图片了。

运营选中一批投稿来 AI 审核的时候，是异步接口，给我一批投稿（最多 100 条），然后提供一个API 用来接受回调，每一个投稿 review 完都会回调一次。一批全部结束还会回调一次。
审核结果里也会返回语音转文字内容/视频描述之类的辅助信息
...
```
Authorization: Bearer <WEBHOOK_TOKEN>      # 与 PLATFORM_API_KEY 是不同的 token
