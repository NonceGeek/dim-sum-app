...
- **LEARNER 不能被分配显式权限** - 只能访问公开语料库
- **RESEARCHER 分配到语料库时自动获得 CREATE 权限** - 可新增和修改词条
- **TAGGER (PARTNER/OUTSOURCING) 分配到语料库时自动获得 WRITE 权限** - 仅可修改词条
- 权限级别由角色决定，管理员只需选择用户和语料库，系统自动确定权限级别
