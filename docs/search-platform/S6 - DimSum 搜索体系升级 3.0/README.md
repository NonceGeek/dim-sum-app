...
S6 数据库基础已于 2026-08-28 在 Production 完成：新增 `content_attribute`、`media_types`，安装自动派生 trigger，完成媒体存量回填，并按高置信来源标记 50,741 条内容属性。现有 Search API 的可选属性过滤、方案 B 媒体过滤和 Entry DTO 兼容扩展已通过 PR #388 合并到 `main`，由团队 Vercel 项目自动部署并完成 `search.aidimsum.com` 线上验收。本期不新建 V3 路由。
