...
| audio 精准结果和详情页 | 原生控件可播放，使用 metadata 预载并保留源文件入口 |
| audio 相关结果列表 | 点击后才创建播放请求；失败给出提示，不预载所有音频 |
| image 精准结果和详情页 | 图片在页面内展示、保持比例、懒加载，并保留源文件入口 |
| image 加载失败 | 显示明确占位，不出现破图且不影响其他结果 |
| image 相关结果列表 | 按钮进入词条详情，不直连带强制下载头的 OSS URL |
...
| 无转写 | 不显示空转写容器，播放器和源文件入口仍可用 |
| 播放失败、404 或慢响应 | 页面不白屏；用户始终可见“打开视频源文件”降级入口 |
| similar/recommended 列表出现视频 | 只显示进入词条详情的视频按钮，不挂载 `<video>`、不预载媒体流 |
| 英文界面 | 播放器、转写和降级入口正常，文案使用英文翻译 |
