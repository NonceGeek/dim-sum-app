...
**开放平台绑定后，53 位关注者均已获取并保存 UnionID。全量结果：14 位绑定成功、34 位未匹配平台账号、5 位冲突、2 条历史记录失败。ECS 每小时增量、每日北京时间 03:20 全量；真实账号回归与 agent 推送仍待验收。**
...
5. **部署定时任务（已完成）**：ECS 上每小时 `--pending`、每日北京时间 03:20 `--full`，使用共享执行锁防止重叠。运行记录见本文末尾。
...
第 2 项的六项配置已覆盖三个环境，生产重新部署与回调自检已完成。用户已确认微信后台回调验证成功。真实关注事件、ECS 白名单与首次同步均已验证，定时器已启用。用户已确认服务号与小程序处于同一开放平台。下一步排查微信未返回 UnionID 的原因，核对两条无效 OpenID，再开展真实账号绑定与推送验收。

### 绑定后最新验收（2026-09-15，北京时间）
