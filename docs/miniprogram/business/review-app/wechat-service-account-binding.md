...
- 合并前全量结果：14 位绑定成功、34 位未匹配、5 位冲突、2 条历史地址失败。用户授权合并三组微信/短信账号后，最新复核为 17 位绑定成功、34 位未匹配、2 位旧地址冲突、2 条历史地址待重试。
...
- 三组跨用户占用已通过事务合并解决，保留短信主用户的角色、历史数据和任务，微信/短信两种登录归到同一用户。需退出重新登录验收；两条无效旧地址与 agent 推送联调仍待处理。执行快照和验收结果见最新明细，未发送消息。
