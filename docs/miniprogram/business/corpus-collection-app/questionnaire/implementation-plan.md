...
- 已完成：五个小程序问卷接口，以及由后台活动配置控制的投稿门禁；全部活动默认开启，投稿与成功事件在同一数据库事务内完成。
...
问卷门禁使用活动表的 `questionnaire_gate_enabled` 配置，现有活动和新活动默认均为 `true`。系统管理员可在 Questionnaire Settings 页面全部开启/关闭，或逐活动调整；投稿接口每次实时读取数据库配置。

发布顺序：

1. 先按 `db pull → 修改 schema → db push` 同步数据库结构，再部署只写不拦截的事件能力。
2. 部署问卷接口并与小程序联调。
3. 如联调期间需要兼容旧版小程序，只在后台临时关闭测试活动门禁；新版联调通过后恢复开启。
4. 校验数据后扩大活动范围。
5. 最后启用后台看板和导出。

回滚时由系统管理员在后台关闭受影响活动的门禁，恢复原活动投稿链路；保留已写档案和事件，不做破坏性回滚。
