...
公开首页、活动和作品读取接口无需登录。投稿、点赞、评论、分享、个人记录等用户态接口使用 `requireMiniprogramAuth`；公开投稿流可选解析登录 Token，以补充当前用户的 `liked` 状态。
...
调用时机：用户正式提交投稿前，由小程序前端先调用平台 `/api/miniprogram/corpus_collection/submissions/precheck` 接口。
...
| verdict | 动作 |
|---------|------|
| `pass` | 小程序继续调用正式投稿接口，投稿进入运营审核队列 |
| `reject` | 小程序提示违规，不继续调用正式投稿接口 |

第一阶段正式投稿和编辑接口不在服务端强制调用 precheck，前端可将 precheck 结果随投稿数据一并提交，平台保存到 `precheck_result` 便于后续追踪。
